#pragma once
#define INI_NAME "LootMenu.ini"

int JLMItems = 5;
UInt32 JLMHidePrompt = 1;
UInt32 JLMWeightVisible = 1;
UInt32 JLMReloadKeyActivates = 1;
float JLMFont = 3.0;
float JLMHeightMin = 32.0;
float JLMHeightMax = 640.0;
float JLMWidthMin = 400.0;
float JLMWidthMax = 640.0;
float JLMOffsetX = 0.6;
float JLMOffsetY = 0.5;
float JLMItemIndent = 8.0;
float JLMIndentTextX = 10.0;
float JLMIndentTextY = 10.0;

float JLMTextAdjust = 8.0;
float JLMJustifyX = 0;
...
void handleIniOptions()
{
	...
	GetModuleFileNameA(LootMenuHandle, iniPath, MAX_PATH);
